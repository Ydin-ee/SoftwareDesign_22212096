...
| 2026-06-10 | 0.01 | Second Draft | 조의현 |
...
정렬 알고리즘(버블, 퀵, 병합 등)과 자료구조(스택, 큐)의 논리적 특성을 전투 스킬의 데미지, 기믹, 쿨타임 등으로 치환하여 명확한 규칙을 가진 전투 환경을 재현한다.
...
**Problem #2 - 무한 반복 루프에서의 메모리 누수 및 UI 렌더링 충돌**
...
더불어, 슬라임과 같은 거대한 적 스프라이트와 배열의 숫자 텍스트가 동시에 렌더링 될 때 발생하는 가독성 저하 및 버튼 클릭 겹침 오류를 방지하기 위해, 유니티 Canvas 계층(Hierarchy)에서 이미지 컴포넌트와 TextMeshPro 객체를 부모-자식 구조로 철저히 분리하여 시각적/기능적 충돌을 원천 차단해야 한다.
