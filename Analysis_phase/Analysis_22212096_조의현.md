...
**GitHub :** https://github.com/Ydin-ee/SoftwareDesign_22212096

## [ Revision history ]
| Revision date | Version # | Description | Author |
| :--- | :--- | :--- | :--- |
| 2026-05-01 | 0.01 | Analysis First Draft | 조의현 |
| 2026-06-10 | 0.02 | Analysis Second Draft | 조의현 |
...
**4. 스킬 시스템 (Skills)**
* `SortingSkill` (및 개별 정렬 스킬 클래스): 실제 정렬 알고리즘(버블, 선택, 삽입 정렬 등) 로직을 연산하고 적 배열 상태를 오름차순에 가깝게 변경시키는 공격 스킬 구현 클래스 
* `DataStructureSkill` (및 QueueShieldSKill): 스택(LIFO) 방어막이나 큐(FIFO) 방어 기믹을 활용하여 플레이어의 생존력을 높이는 유틸리티 스킬 구현 클래스
...
- **설명:** 플레이어가 게임을 처음 실행했을 때 진입하는 화면, 가운데에 "알고리즘 용사" 로고가 배치되며, 하단에 Game Start 및 Quit 버튼이 위치한다. 좌측 상단에는 게임의 스토리와 조작 방법, 승리 조건을 알려주는 '[?] 도움말' 버튼이 배치되어 있다.
...
### 4.3. 
![Reward Screen](./RewardScreen.png)
- **설명:** 전투 승리 시 기존 전투 화면 위에 반투명한 패널로 팝업된다. 3장의 보상 카드 중 하나를 마우스로 클릭하여 캐릭터 업그레이드를 진행한다. 스킬 진화를 선택할 경우 별도의 진화 패널로 이동하며, 모든 스킬이 만렙일 경우 선택이 차단된다. 진화 패널에서는 '돌아가기' 버튼을 통해 언제든 이전 보상 창으로 복귀할 수 있다.
