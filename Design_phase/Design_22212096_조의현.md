...
3. **ArrayVisualizer (데이터 시각화)**
	- 적이 가진 난수 배열 정보를 참조하여 화면에 UI 블록을 동적으로 배치하고, 정렬 연산 시 이동 애니메이션을 제어한다.
4. **SkillBase / SortingSkill / DataStructureSkill (상속 구조)**
	- 공통 속성은 부모 추상 클래스에 은닉하고, 구체적인 정렬 연산(Bubble, Quick) 및 자료구조 기믹(Stack Shield)은 상속을 통해 다형성(Polymorphism)을 구현한다.
...
| **추상 메서드 (Abstract Method)** | 부모 클래스에서 선언만 하고 실제 구현은 상속받는 자식 클래스에서 강제하도록 하는 메서드 설계 기법 (`SkillBase.Execute()`). |
