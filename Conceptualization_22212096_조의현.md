...
버블 정렬[O(N^2)]과 퀵 정렬[O(NlogN)]을 단순히 데미지로만 치환하면, 효율이 좋은 스킬만 남발하는 밸런스 붕괴 현상이 발생한다. 이를 방지하기 위해 퀵 정렬에 '최대 5회 사용 제한' 같은 페널티를 주거나, 상대의 '정렬 상태'에 따라 데미지가 달라지는 기믹(병합 정렬)을 추가한다. 또한 보상을 통한 '정렬 기능 개선' 시스템을 도입하여 하위 스킬도 버려지지 않고 콤보나 시너지 용도로 쓰이게끔 설계해야 한다.
